...
async function countCases(url: string, key: string, riskLevel: RiskLevel) {
...
export async function GET(request: Request) {
  ...
  const [totalCases, urgentCases, emergencyCases] = await Promise.all([
    ...
    countCases(url, key, "emergency")
  ]);

  if (totalCases === null || urgentCases === null || emergencyCases === null) return fail(502, "UPSTREAM_ERROR", "Failed to query case metrics", requestId);
